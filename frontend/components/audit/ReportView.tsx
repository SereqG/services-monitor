    ...
    return (
      <div className="rounded-xl border border-destructive/40 bg-destructive/5 p-5">
        <div className="mb-1 font-mono text-[10px] uppercase tracking-widest text-muted-foreground">
          {dict.categories[category.name] ?? category.name}
        </div>
...
export function ReportView({ report }: { report: AuditReport }) {
  ...
  if (health !== null) {
    ...
    coreMetrics.push({
      label: "TTFB",
      value: ttfb !== null ? `${ttfb.toFixed(0)}ms` : dict.common.na,
      description: dict.report.metrics.ttfbDescription,
  ...
  if (seo !== null) {
    coreMetrics.push({
      label: "SEO",
      value: `${seo.score}`,
      description: dict.report.metrics.seoDescription,
      fill: seo.score,
  ...
  if (accessibility !== null) {
    coreMetrics.push({
      ...
      description: dict.report.metrics.a11yDescription,
  ...
  return (
    <section className="mt-20 space-y-12">
      ...
      <div className="flex flex-col items-start justify-between gap-3 border-b border-border pb-6 md:flex-row md:items-end">
        ...
        {health !== null && (
          <div className="md:text-right">
            ...
            >
              {health.is_available ? dict.common.available : dict.common.unavailable} · HTTP{" "}
              {health.status_code ?? "—"}
            </div>
          </div>
        )}
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-4">
        <div className="flex flex-col items-center justify-center rounded-xl border border-border bg-card p-8 text-center">
          {overall !== null ? (
            ...
            <div className="font-mono text-4xl font-bold text-muted-foreground">{dict.common.na}</div>
          )}
      ...
      <div className="space-y-4">
        <h3 className="font-mono text-xs uppercase tracking-widest text-muted-foreground">
          {dict.report.discoverySummary}
        </h3>
        <div className="overflow-hidden rounded-xl border border-border bg-card/50">
          <ul className="divide-y divide-border">
            <StatRow label={dict.report.pagesDiscovered} value={String(discovery.total_discovered)} />
            <StatRow label={dict.report.pagesAvailable} value={String(discovery.total_allowed)} />
            <StatRow label={dict.report.blockedByRobots} value={String(blockedCount)} />
            <StatRow label={dict.report.crawlDuration} value={`${discovery.duration_seconds.toFixed(2)}s`} />
            <StatRow
              label={dict.report.sitemapFound}
              value={discovery.robots_policy.sitemap_urls.length > 0 ? dict.common.yes : dict.common.no}
            />
            <StatRow
              label={dict.report.robotsFetched}
              value={discovery.robots_policy.fetched ? dict.common.yes : dict.common.no}
            />
          </ul>
        </div>
        {discovery.hit_limit && (
          <div className="rounded-lg border border-warning/40 bg-warning/10 px-4 py-3 text-xs text-warning">
            {dict.report.crawlLimitNote}
          </div>
        )}
      </div>
      ...
      {report.page_results.length > 0 && (
        <div className="space-y-4">
          <h3 className="font-mono text-xs uppercase tracking-widest text-muted-foreground">
            {dict.report.pagesAudited(report.page_results.length)}
          </h3>
